...
public class Hotel
{
    ...
    public Dictionary<int, List<Dictionary<int, int>>> GuestConfigurations = new();
    ...
    private static List<Dictionary<int, int>> GetConfigs(List<int> rooms, Dictionary<int, int> numRooms, int numPeople)
    {
        ...
        if (rooms.Count == 0) return configs;

        if (rooms.Count == 1)
        {
            for (var i = 0; i <= numRooms[rooms[0]]; i++)
        ...
        for (var i = 0; i <= numRooms[rooms[0]]; i++)
        {
            ...
            foreach (var rc in recursiveConfigs)
                if (!existingRecursiveConfigs.Any(c => c.SequenceEqual(rc)))
                {
                    ...
                    foreach (var kvp in rc) newConfig[kvp.Key] = kvp.Value;
...
public class Room
{
    ...
    public List<Tuple<DateTime, DateTime>> GetAvailability(DateTime rangeStart, DateTime rangeEnd, int nRooms,
        int minLength = 0)
    {
        ...
        if (minLength > rangeDays || minLength == 0) minLength = rangeDays;
        ...
        while (idx <= rangeDays - minLength)
        {
            ...
            var endIdx = idx + minLength;
            while (endIdx < rangeDays && freeRooms[endIdx] >= nRooms) endIdx++;
    ...
    public int[] GetFreeRooms(DateTime rangeStart, DateTime rangeEnd)
    {
        ...
        foreach (var reservation in Bookings)
        {
            ...
            if (resStart <= resEnd)
                for (var i = 0; i <= (resEnd - resStart).TotalDays; i++)
